...
struct Serpent * creer_serpent_2(int n, int m) {
	...
	/* Initialisation des coordonnées du serpent2 dans la grille */	
	serp->cordx = (2*(n/4));
	serp->cordy = (2*(m/4));
	
	/* Création de la liste de sections formant le serpent2 */
	serp->chaine = creer_liste();
	ajout_debut_liste(serp->chaine, creer_section(1, 1));
	
	/* Création de la liste de mouvements du serpent2 */
	serp->mouvement = creer_liste_mouvement();
	ajout_debut_liste_mouvement(serp->mouvement, creer_case(serp->cordx, serp->cordy, NEUTRE));  
